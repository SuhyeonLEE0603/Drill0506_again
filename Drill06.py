...
def set_new_target_arrow():
    ...
    if points:  # points 리스트 안에 남아있는 점이 있으면
        ...
        action = 3 if action == 1 else 2  # 이전에 소년이 우측으로 이동중 이었으면, IDLE 동작시 우측을 바라보도록
        frame = 0
        target_exists = False
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            cx = (1 - t) * sx + t * hx  # cx 는 시작 x와 1-t:t의 비율로 섞은 위치
            cy = (1 - t) * sy + t * hy
            t += 0.001
        else:  # 목표지점에 도달하면
...
while running:
    ...
    handle_events()  # 사용자의 입력을 받음
    update_world()  # world 안의 객체들의 상호 작용을 계산하고 그 결과를 update함
